...
- `calculateWithNeighbours` currently writes all nine chunks and darkens the eight ring chunks. Task 3 deliberately does not repeat that. Once this plan is done, that method could be reduced to writing only its centre chunk — out of scope here, tracked in `STATUS.md`. *(Done after this plan closed: it now writes only its centre chunk. `STATUS.md` records that under* Defects found and fixed *rather than under* Open*, so the pointer above no longer resolves there.)*
